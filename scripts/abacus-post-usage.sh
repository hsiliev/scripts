...
TOKEN=$(curl --user $CLIENT_ID:$CLIENT_SECRET -s "$AUTH_SERVER/oauth/token?grant_type=client_credentials&scope=abacus.usage.iotae.write%20abacus.usage.iotae.read" | jq -r .access_token)
