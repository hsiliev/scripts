...
while true; do
  ...
  curl -f -H "Authorization: bearer $TOKEN" -H "Content-Type: application/json" -X POST -d $BODY --write-out "%{http_code}." $URL
  ...
  if [ "$(uname)" != "Darwin" ]; then
    ...
    echo ""
    curl -i -H "Authorization: bearer $TOKEN" -H "Content-Type: application/json" -X POST -d $BODY --write-out "%{http_code}." $URL
  fi
done
