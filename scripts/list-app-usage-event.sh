...
if [ $show_all = 1 ]; then
  if [ $filter_org = 1 ]; then
    ...
    curl -sk -H "Authorization: bearer $TOKEN" -H "Content-Type: application/json" "$API/v2/app_usage_events?results-per-page=10000&page=$page" | jq ".resources[].entity | select(.org_guid == \"$ORG_GUID\")"
  else
